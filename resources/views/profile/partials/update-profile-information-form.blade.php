    ...
    <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        <div class="flex flex-col items-center justify-center mb-8">
            <div class="relative group cursor-pointer">
                @if ($user->avatar_url)
                    <img src="{{ $user->avatar_url }}" alt="Avatar" class="h-28 w-28 shrink-0 rounded-full object-cover border-4 border-white shadow-lg">
                @else
                    <div class="h-28 w-28 shrink-0 rounded-full bg-white text-gray-400 flex items-center justify-center border-4 border-white shadow-lg overflow-hidden">
                ...
                <input type="file" id="avatar" name="avatar" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/*">
            </div>
            <p class="mt-2 text-sm text-gray-500 text-center">Click to change profile photo</p>
